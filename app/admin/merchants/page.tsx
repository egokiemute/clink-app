...
export default function AdminMerchantsPage() {
  ...
  async function handleReject(id: string, note: string) {
    ...
    try {
      ...
      setMerchants((prev) =>
        prev.map((m) =>
          m.id === id ? { ...m, verificationStatus: "rejected", verificationNote: note } : m
        )
      );
      setRejectModal(null);
      setRejectNote("");
  ...
  const pendingCount = merchants.filter((m) => m.verificationStatus === "pending").length;
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="space-y-2 animate-pulse">
          {[1, 2, 3].map((i) => <div key={i} className="h-20 rounded-2xl bg-surface-strong" />)}
        </div>
      ) : merchants.length === 0 ? (
        <div className="rounded-2xl border border-border bg-white px-6 py-12 text-center">
          <p className="text-sm text-primary/40">No merchants in this category.</p>
        </div>
      ) : (
          ...
          {merchants.map((m, i) => (
            ...
            >
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {m.verificationStatus && (
                        <span className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${STATUS_BADGE[m.verificationStatus] ?? "bg-surface-strong text-primary/50"}`}>
                          {m.verificationStatus}
                        </span>
                      )}
                    </div>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  {m.verificationStatus === "pending" && (
                    <>
                      ...
                      <button
                        onClick={() => { setRejectModal({ id: m.id, name: m.name }); setRejectNote(""); }}
                        disabled={actionLoading === m.id}
              ...
              {(m.businessName || m.country || m.businessType) && (
                <div className="mt-2.5 ml-13 flex flex-wrap gap-x-4 gap-y-1 pl-[52px] text-xs text-primary/45">
                  ...
                  <span>Joined {new Date(m.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}</span>
                </div>
              )}

              {m.verificationNote && m.verificationStatus === "rejected" && (
                <p className="mt-2 ml-[52px] text-xs text-red-600 italic">{m.verificationNote}</p>
              )}
            </div>
          ))}
        </div>
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-white px-6 py-6 shadow-xl">
            ...
            <p className="mt-1 text-sm text-primary/50">Provide a reason that will be emailed to the merchant.</p>
            <textarea
              value={rejectNote}
