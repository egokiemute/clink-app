    ...
    <div className="space-y-6 text-center">
      <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-secondary">
        <svg width="30" height="30" viewBox="0 0 30 30" fill="none" aria-hidden="true">
          <path d="M7 15l6 6 10-10" stroke="#252f2c" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>

      <div>
        <h2 className="text-2xl font-medium tracking-[-0.04em] text-primary">Payment complete</h2>
        <p className="mt-1 text-sm text-primary/55">
          {payment.amount} {payment.currency} received
          {payment.localAmount != null && (
            <> &rarr; <span className="font-medium text-primary">{payment.localAmount.toLocaleString()} {payment.localCurrency}</span></>
          )}
        </p>
      </div>
      ...
      {payment.successUrl && (
        <div className="pt-2">
          ...
          <p className="mt-3 text-xs text-primary/40">Redirecting automatically&hellip;</p>
        </div>
      )}
    </div>
  );
